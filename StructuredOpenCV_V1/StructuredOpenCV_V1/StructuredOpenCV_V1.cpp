...
int main(int, char**)
{	
	//open: m_bPhase(fringes)=true to use unwrapp to decode phase shift
	//COptions* options = new COptions(1024, 768, 10,4,true,true,true,false);
	COptions* options = new COptions(1024, 768, 10, 4, true, true, true, true);
	...
	//Yang: Goal: to compare normal calibration method with improved calibration method;
	//Yang: This is only done for one camera calibration without using structured light.
	//cal->Calibrate("in_calib.xml", camera, dist); 
	//Yang: Alternative improved calibration method from paper;
	...
	*/
	...
	//decoder->Generate3Dpoints(camera, dist);
