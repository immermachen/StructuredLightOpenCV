...
bool CDecoder::Decode(float thres, vector<Mat>& vCaptures)
{
	...
	if (m_Info->m_bHorizontal)
	{
		...
		tmp = m_mMask[0].clone();
		temp.push_back(tmp);
		imagenames.push_back("m_mMask0");

		tmp = m_mGrayError[0].clone();
		temp.push_back(tmp);
		imagenames.push_back("m_mErrorGray0");

#endif 
		if (m_Info->m_nNumFringes>0)
		{
	...
	if (m_Info->m_bVertical)
	{
		...
		tmp = m_mMask[1].clone();
		temp.push_back(tmp);
		imagenames.push_back("m_mMask1");

		tmp = m_mGrayError[1].clone();
		temp.push_back(tmp);
		imagenames.push_back("m_mErrorGray1");
	...
	cv::imwrite("m_mMask0.bmp", m_mMask[0]);
	cv::imwrite("m_mMask1.bmp", m_mMask[1]);
	cv::imwrite("m_mReliableMask0.bmp", m_mReliableMask[0]);
	cv::imwrite("m_mReliableMask1.bmp", m_mReliableMask[1]);
	...
	if (m_Info->m_bHorizontal&&m_Info->m_bVertical)
	{
		for (int y = 0; y < m_mMask[1].cols; y++)
			for (int x = 0; x < m_mMask[1].rows; x++)
			{
				...
				m_mReliableMask[0].at<float>(x, y) = std::min(m_mReliableMask[0].at<float>(x, y), m_mReliableMask[1].at<float>(x, y));
			}

		cv::imwrite("m_mReliableMask_merged.bmp", m_mReliableMask[0]);
		//apply Mask:
		for (int y = 0; y < m_mMask[1].cols; y++)
			for (int x = 0; x < m_mMask[1].rows; x++)
	...
	if (m_Info->m_bHorizontal&&m_Info->m_bVertical)
	{
		...
		//minMaxIdx(diff1, &minValComp, &maxValComp);
		//float BrightesPixel = std::max(maxVal, maxValComp);

		//diff0.convertTo(tmp, CV_8UC1, 255 / BrightesPixel, 0);
		//temp.push_back(diff0);
		//imagenames.push_back("m_mDiff0");

		//diff1.convertTo(diff1, CV_8UC1, 255 / BrightesPixel, 0);
		//temp.push_back(diff1);
		//imagenames.push_back("m_mDiff1");
	...
	cap.SerializeCaptures(temp, imagenames);
#endif

	if (m_Info->m_nNumFringes>0) //Yang: for calibration function, it use m_mPhaseMap;
	{
	...
	tmp = m_mGray[0].clone();
	minMaxIdx(tmp, &minVal, &maxVal);
	std::cout << "m_mGray0 rows=" << tmp.rows << ", cols=" << tmp.cols << ",minVal=" << minVal << ", maxVal=" << maxVal << endl;	
	string filename = "m_mGray0.txt";
	CCapturador::writeMatToFile(tmp, filename.c_str(),0);
	tmp.convertTo(tmp, CV_8UC1, 255 / maxVal, 0);
	cv::imwrite("m_mGray0.bmp", tmp);

	tmp = m_mGray[1].clone();
	minMaxIdx(tmp, &minVal, &maxVal);
	std::cout << "m_mGray1 rows=" << tmp.rows << ", cols=" << tmp.cols << ",minVal=" << minVal << ", maxVal=" << maxVal << endl;
	filename = "m_mGray1.txt";
	CCapturador::writeMatToFile(tmp, filename.c_str(),1);	
	tmp.convertTo(tmp, CV_8UC1, 255 / maxVal, 0);
	cv::imwrite("m_mGray1.bmp", tmp);

	//cv::imwrite("m_mGrayError0.bmp", m_mGrayError[0]);
	...
	////use mask
	//Mat m_mGray0 = MaskMat(m_mGray[0], m_mReliableMask[0], true);
	//Mat m_mGray1 = MaskMat(m_mGray[1], m_mReliableMask[0], true);	
	//filename = "m_mGray0_Masked.txt";
	//CCapturador::writeMatToFile(m_mGray0, filename.c_str(),0);
	//filename = "m_mGray1_Masked.txt";
	//CCapturador::writeMatToFile(m_mGray1, filename.c_str(),1);

	//m_mGray0.convertTo(m_mGray0, CV_8UC1, 255 / maxVal, 0);
...
void CDecoder::DecodeGray(int dir,float int_threshold)
{
	...
	for (int i = 0; i < nbits; i++)
	{
		if (m_Info->m_bComplementary)
		{
			...
			//Yang
			int numphase = 0;
			if (m_Info->m_nNumFringes>0 && dir == 1)
			{
...
void CDecoder::UnwrapPhase(Mat& phase, int period, Mat& reference, Mat& result, Mat& unwrap_error)
{
	...
	for (int x = 0; x < phase.rows; x++)
	{
		...
		if (diff < window)
		{
			float newval = graycode - (graycode % period) + period * moire_phase; //e.g.: 11-(11%4)+4*0.3 = 9.2
			if (newval < 0)   //TODO: Yang: check this:::: how to deal with negative value???
			{
				result.at<float>(x, y) = graycode; 
			}
			else
